...
static void check_timed_weapon_drops(void)
{
  for (int i = 0; i < TIMED_DROP_COUNT; i++) {
    ...
    WeaponType_t pool[8];
    int pool_count = 0;
    WeaponType_t all_weapons[] = {WEAPON_WAND, WEAPON_SPIN, WEAPON_CHAIN, WEAPON_ORBIT, WEAPON_BOMB, WEAPON_TURRET, WEAPON_TRAIL, WEAPON_SCYTHE};
    for (int w = 0; w < 8; w++) {
      if (!weapons_has(all_weapons[w]) && !drops_has_type(all_weapons[w]) && !weapons_is_type_stolen(all_weapons[w]))
...
static float next_beholder_time = BEHOLDER_FIRST_SPAWN_TIME;
...
#define BEHOLDER_DIR_BASE_HP      9

// Mimic spawn schedule
#define MIMIC_FIRST_SPAWN_TIME  180.0f
#define MIMIC_SPAWN_INTERVAL    180.0f
#define MIMIC_DIR_BASE_HP        40
#define MIMIC_DIR_HP_GROWTH       5
static float next_mimic_time = MIMIC_FIRST_SPAWN_TIME;
static int mimic_spawn_count = 0;
static int mimic_discovered_this_run = 0;
...
void director_init(void)
{
  ...
  next_beholder_time = BEHOLDER_FIRST_SPAWN_TIME;
  beholder_spawn_count = 0;
  beholder_discovered_this_run = 0;

  next_mimic_time = MIMIC_FIRST_SPAWN_TIME;
  mimic_spawn_count = 0;
  mimic_discovered_this_run = 0;
}
...
void director_update(float dt)
{
  ...
  // Mimic spawn (every 180s starting at 180s)
  if (game_elapsed_timer >= next_mimic_time && weapons_get_count() > 0) {
    if (!mimic_discovered_this_run) {
      mimic_discovered_this_run = 1;
      if (!save_data_get_int("mimic_discovered", 0)) {
        save_data_set_int("mimic_discovered", 1);
        save_data_flush();
      }
    }
    int hp = MIMIC_DIR_BASE_HP + MIMIC_DIR_HP_GROWTH * mimic_spawn_count;
    mimic_spawn(hp, mimic_spawn_count);
    mimic_spawn_count++;
    next_mimic_time += MIMIC_SPAWN_INTERVAL;
  }
