...
{
  // Turret bullet collision is now handled inside turret_update() in weapons.c
  // to support pierce (bullets passing through multiple enemies).
}
...
void collision_resolve_deaths(void)
{
  for (int j = 0; j < max_enemies; j++)
  {
    if (was_alive[j] && enemy_get_state(j) == ENEMY_STATE_CORPSE)
    {
      ...
      WeaponType_t kill_source = enemy_get_last_hit_source(j);
      stats_record_weapon_kill(kill_source);
      weapons_notify_kill(kill_source);

      // Boss speed-kill check (beholder/mimic — snake handled in snake_kill)
      {
        EnemyType_t et = enemy_get_type(j);
        if (et == ENEMY_TYPE_BEHOLDER || et == ENEMY_TYPE_MIMIC)
        {
          if (director_get_elapsed() - enemy_get_spawn_time(j) <= 30.0f)
            achievements_notify_boss_speedkill((int)et);
        }
      }
    }
  }
