  ...
  for (int i = 0; i < MAX_PICKUPS; i++) {
    if (!pickups[i].active) {
      pickups[i] = (Pickup_t){
        .x = x, .y = y,
        .type = type,
        .lifetime = PICKUP_LIFETIME + global_get_pickup_lifetime_bonus(),
        .active = 1,
        .age = 0.0f
      };
      return;
...
void pickups_update(float dt)
{
  for (int i = 0; i < MAX_PICKUPS; i++) {
    ...
    pickups[i].age += dt;
    if (pickups[i].age >= pickups[i].lifetime) {
      pickups[i].active = 0;
    }
  }
}
...
void pickups_draw(void)
{
  for (int i = 0; i < MAX_PICKUPS; i++) {
    ...
    if (pickups[i].age >= pickups[i].lifetime - 3.0f) {
...
void pickups_check_pity(float dt, float player_x, float player_y)
{
  ...
  if (pity_timer <= 0.0f) {
    ...
    float reduced = PITY_TIMER_DEFAULT - global_get_pity_reduction();
    if (reduced < 5.0f) reduced = 5.0f;
    pity_timer = reduced;
  }
}
...
{
  return PICKUP_COLLECT_DIST + global_get_pickup_magnet_bonus();
}
