...
void hud_draw_game(float time_remaining)
{
  ...
  // Draw player health bar (top center)
  {
    ...
    static const int bar_widths[4]  = { 240, 264, 288, 312 };
    static const int bar_heights[4] = { 18,  21,  24,  27  };
    ...
    if ( shielded )
    {
      ...
      if (start > shield_max) shield_max = start;
  ...
  // Draw dash indicator (bottom left)
  {
    ...
    int dash_h = 54;
    int dash_x = 28;
    int dash_y = SCREEN_HEIGHT - dash_h - 52;
  ...
  // XP bar — full width at bottom of screen
  {
    ...
    a_DrawText( lv_text, SCREEN_WIDTH - 22, xp_bar_y - 44, lv_style );
    ...
    a_DrawText( xp_text, SCREEN_WIDTH - 22, xp_bar_y - 27, xp_style );
  }
}
