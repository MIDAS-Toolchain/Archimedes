...
#define XP_MAGNET_RADIUS  110.0f
#define XP_MAGNET_SPEED   400.0f
#define XP_COLLECT_DIST    20.0f
...
void xp_init(void)
{
  for (int i = 0; i < MAX_XP_ORBS; i++)
    orbs[i].active = 0;
  current_xp = global_get_starting_xp();
  current_level = 0;
  pending_level_up = 0;
...
void xp_spawn_orbs(float death_x, float death_y, int enemy_type)
{
  ...
  else if (enemy_type == ENEMY_TYPE_MIMIC) count = 8;

  if (global_has_bounty()) count += 1;
...
void xp_update(float dt)
{
  ...
  for (int i = 0; i < MAX_XP_ORBS; i++) {
    ...
    if (dist < magnet_r && player_is_alive())
      orbs[i].attracted = 1;

    if (orbs[i].attracted && dist > 0.1f && player_is_alive()) {
      orbs[i].attract_time += dt;
      float t = 1.0f - dist / magnet_r;
      if (t < 0.0f) t = 0.0f;
      float speed_mult = 1.0f + orbs[i].attract_time * 2.0f;
      float pull = XP_MAGNET_SPEED * (0.3f + t * t) * speed_mult;
      float nx = dx / dist;
      float ny = dy / dist;
      orbs[i].x += nx * pull * dt;
      orbs[i].y += ny * pull * dt;
